	...
	class WPEX_Theme_Options {
		...
		public static function sanitize( $options ) {
			...
			if ( $options ) {
                ...
                if ( ! empty( $options['mapurl'] ) ) {
					$options['mapurl'] = sanitize_text_field( $options['mapurl'] );
				} else {
					unset( $options['mapurl'] ); // Remove from options if empty
				}
			...
			<div class="wrap">
				...
				<form method="post" action="options.php">
					...
					<table class="form-table wpex-custom-admin-login-table">
                        ...
                        <tr valign="top">
							<th scope="row"><?php esc_html_e( 'Map Url', 'text-domain' ); ?></th>
							<td>
								<?php $value = self::get_theme_option( 'mapurl' ); ?>
								<input type="text" name="theme_options[mapurl]" value="<?php echo esc_attr( $value ); ?>">
							</td>
						</tr>
                        <tr valign="top">
